...
// 28RE用opening_message生成のsystemInstruction（再オファー・本人の強みのみ）
const SYSTEM_INSTRUCTION_28RE_OPENING = `あなたは新卒スカウト文の「個別訴求パート」だけを作るライターです。
${SYSTEM_INSTRUCTION_BASE}

【重要】
あなたが生成するのは「本人の強み」を伝える部分のみです。
この前後に置く導入文（連絡した理由）と受け文（当社の仕事との接続）、
および会社紹介などの固定文はアプリ側で別途結合します。
${JSON_INSTRUCTION}`;

// 28RE用opening_message生成のプロンプト（再オファー｜本人の強みのみ・前後を固定文が挟む）
const OPENING_28RE_TEMPLATE = `【タスク】
以下のOfferBox貼り付けテキストを読み、学生本人の「強み」を伝える文章を作成してください。
この文章は、一度オファーを送った学生への再オファー文の一部として使われます。
...
■ 生成する内容（最重要）
・書くのは「本人の強み」の記述のみ
・2〜3文でまとめる（全体で100〜180文字程度）
・自己PR・学生時代の取り組み・アルバイト・部活動・研究・資格など、
  入力された全情報から具体的な事実（数字・固有名詞・行動）を必ず1つは入れる
・「事実 → そこから見える強み」の流れで書く

■ 絶対に書かないこと（前後の固定文が担うため）
・連絡した理由・オファーの趣旨
　（例：「ご連絡しました」「ご連絡いたしました」「オファーをお送りしました」
　　「お話ししたく」「改めてご連絡」などの締めの文は禁止）
・「再度」「改めて」「前回」など、再連絡であることに触れる表現
・当社の仕事・事業・職種への接続
　（直後に固定文「当社の仕事に活かしていただける強みだと感じています。」が続くため）
・会社紹介・イベント案内・承諾のお願い

■ 文末（最重要）
・最終文の直後に「当社の仕事に活かしていただける強みだと感じています。」が続いても
  自然につながる文末にすること
・具体的には「〜が印象に残りました。」「〜に目が留まりました。」
  「〜が伝わってきました。」「〜と感じました。」などで終える
・「〜ました」の同じ文末を連続で繰り返さない
...
・学生の希望職種・希望業界・希望企業規模・志向への言及

【良い例1】
自己PRを拝見し、飲食のピーク時にも周囲を見て優先順位を判断する冷静さに目が留まりました。
分からないことをその場で確認し、次から自分で動けるようにしている点も伝わってきました。

【良い例2】
TOEIC学習への取り組みを拝見し、毎朝コツコツ続けて710点まで伸ばした粘り強さが印象に残りました。
弱点を見つけて改善していく進め方は、人と関わる場面でも活きる強みだと感じました。

【悪い例】
・「〜と考え、ご連絡しました。」（連絡理由・締めの文は禁止）
・「改めてご案内したく〜」（再連絡に触れるのは禁止）
・「当社のコンサル営業で活かせると思います。」（当社との接続は固定文が担う）
...
export async function POST(request: NextRequest) {
  try {
    ...
    // mode検証
    if (!mode || !["title", "opening", "opening_re", "opening_28d", "b_profile_line"].includes(mode)) {
      return NextResponse.json(
        { error: "mode must be 'title', 'opening', 'opening_re', 'opening_28d', or 'b_profile_line'" },
        { status: 400 }
      );
    }
