...
以下のOfferBox貼り付けテキストから、スカウト文の冒頭メッセージ（opening_message）を生成してください。

【文章構成（この順番で書く）】
1. 冒頭で本人の強みを1文で要約する
2. その根拠となる具体エピソードを1〜2文で説明
3. 当社（HR業界、人材支援）との親和性を示す接続文
4. 必ず「ぜひ一度お話したくご連絡しました！」で締める

【文章条件】
- 200〜300文字（最大300文字）。300文字を超えたら必ず短くする
- 具体エピソードを最低1つ含める（入力テキストから拾う）
- 「」（かぎ括弧）は使わない
- **（アスタリスク装飾）は使わない
...
【出力例のイメージ】
周囲を巻き込みながら主体的に行動できる方だと感じました。
サークルで新入生歓迎イベントを企画し、例年の2倍の参加者を集めた経験からも、
目標に向けて周りを動かす力をお持ちだと拝見しました。
人の可能性を広げる当社の仕事にも、その力が活かせると思い、
ぜひ一度お話したくご連絡しました！
